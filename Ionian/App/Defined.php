...
class Defined extends App {
    ...
    protected function addRoute($method, $uri, $target, $preq = null) {
        ...
        if (!is_string($target) || !is_string($uri)) {
            throw new \Exception("Route URI and TARGET parameters must both be strings!");
        }
        ...
        if (isset($this->routes[$method][$uri])) {
            throw new \Exception("Path $uri already defined!");
        }
        ...
        if (($at === false) || ($at === 0) || ($at === strlen($target) - 1)) {
            throw new \Exception("$target for $uri is not a valid target!");
        }
